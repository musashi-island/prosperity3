...
class Trader:
    ...
    def ATM_voucher_orders(self, product, order_depth, position, traderObject, timestamp):
        ...
        diff = IV - short_ma
        logger.print(f"IV: {IV:.5f}, SMA100: {short_ma:.5f}, SMA200: {long_ma:.5f}, Spread: {diff:.5f}")

        trade_signal = 0

        # Mean Reversion Strategy
        if diff > IV_DEVIATION_THRESHOLD[product]:
            logger.print("IV above long MA — SELL signal")
            trade_signal = -1
        elif diff < -IV_DEVIATION_THRESHOLD[product]:
            logger.print("IV below long MA — BUY signal")
            trade_signal = 1
        ...
        if trade_signal:
            ...
            logger.print(f"Placing order: {'BUY' if trade_signal == 1 else 'SELL'} {abs(vol)} @ {price}")
    ...
    def run(self, state: TradingState):
        ...
        if all(p in od for p in required_basket):
            ...
            result[Product.CROISSANTS] = orders[Product.CROISSANTS]
