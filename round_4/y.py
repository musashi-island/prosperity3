...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if state.traderData:
            try:
                full = jsonpickle.decode(state.traderData)
            except:
                pass
        ...
        # --- VOUCHER_10000 logic ---
        opt = full["options"]
        vp = Product.VOLCANIC_ROCK_VOUCHER_10000
        opt.setdefault(vp, {"past_iv": [], "past_gamma": [], "past_vanna": []})
        for k in ["past_iv","past_gamma","past_vanna"]:
            opt[vp].setdefault(k, [])
        if vp in self.params_opt and vp in state.order_depths and Product.VOLCANIC_ROCK in state.order_depths:
            od_v = state.order_depths[vp]
            od_u = state.order_depths[Product.VOLCANIC_ROCK]
            if od_v.buy_orders and od_v.sell_orders and od_u.buy_orders and od_u.sell_orders:
                ba, bb = min(od_v.sell_orders), max(od_v.buy_orders)
                mid = (ba+bb)/2
                uba, ubb = min(od_u.sell_orders), max(od_u.buy_orders)
                umid = (uba+ubb)/2
                tte = max(5/365 - state.timestamp/1_000_000/365, 1e-9)
                iv = BlackScholes.implied_volatility(mid, umid, self.params_opt[vp]["strike"], tte)
                g  = BlackScholes.gamma(umid, self.params_opt[vp]["strike"], tte, iv)
                va = BlackScholes.vanna(umid, self.params_opt[vp]["strike"], tte, iv)
                w = self.params_opt[vp]["vol_window"]
                for hist_key, val in [("past_iv",iv),("past_gamma",g),("past_vanna",va)]:
                    h = opt[vp][hist_key]; h.append(val)
                    if len(h)>w: opt[vp][hist_key] = h[-w:]
                ivh, gh, vah = opt[vp]["past_iv"], opt[vp]["past_gamma"], opt[vp]["past_vanna"]
                if len(ivh)>=w:
                    miv, siv = np.mean(ivh), np.std(ivh)
                    mg, mav = np.mean(gh), np.mean(np.abs(vah))
                    fair = BlackScholes.black_scholes_call(umid, self.params_opt[vp]["strike"], tte, miv)
                    logger.print(f"[{vp}] Mid={mid:.2f}, Fair={fair:.2f}, IV={iv:.4f}")
                    low = iv < (miv - IV_STD_DEV_MULTIPLIER*max(siv, MIN_IV_STD_DEV_THRESHOLD))
                    high= iv > (miv + IV_STD_DEV_MULTIPLIER*max(siv, MIN_IV_STD_DEV_THRESHOLD))
                    pb = mid < fair; pa = mid > fair
                    gok = g > GAMMA_MEAN_FACTOR*mg; vok=abs(va)>VANNA_MEAN_FACTOR*mav
                    sig = 0
                    if low and pb and gok and vok: sig = -1
                    elif high and pa and gok and vok: sig = 1
                    elif (low or high or pb or pa):
                        rr=[]
                        if not gok: rr.append("Gamma low")
                        if not vok: rr.append("Vanna low")
                        logger.print(f"[{vp}] Suppressed: {','.join(rr)}")
                    if sig:
                        ords=[]; pos = state.position.get(vp,0); lim=self.LIMIT_OPT[vp]
                        if sig==-1:
                            need = lim - pos
                            price = ba
                            avail = sum(abs(v) for p,v in od_v.sell_orders.items() if p<=price)
                            q = min(need, avail)
                            if q>0:
                                ords.append(Order(vp, price, q))
                                logger.print(f"BUY {vp}: {q}@{price}")
                        else:
                            need = abs(-lim - pos)
                            price = bb
                            avail = sum(abs(v) for p,v in od_v.buy_orders.items() if p>=price)
                            q = min(need, avail)
                            if q>0:
                                ords.append(Order(vp, price, -q))
                                logger.print(f"SELL {vp}: {q}@{price}")
                        if ords: combined[vp] = ords
                else:
                    logger.print(f"[{vp}] Warming up ({len(ivh)}/{w})")
            else:
                logger.print(f"[{vp}] Skipped: missing orders")

        # --- VOUCHER_9750 logic ---
        vp2 = Product.VOLCANIC_ROCK_VOUCHER_9750
        opt.setdefault(vp2, {"past_iv": [], "past_gamma": [], "past_vanna": []})
        for k in ["past_iv","past_gamma","past_vanna"]:
            opt[vp2].setdefault(k, [])
        if vp2 in self.params_opt and vp2 in state.order_depths and Product.VOLCANIC_ROCK in state.order_depths:
            od_v = state.order_depths[vp2]
            od_u = state.order_depths[Product.VOLCANIC_ROCK]
            if od_v.buy_orders and od_v.sell_orders and od_u.buy_orders and od_u.sell_orders:
                ba, bb = min(od_v.sell_orders), max(od_v.buy_orders)
                mid = (ba+bb)/2
                ...
                umid = (uba+ubb)/2
                tte = max(5/365 - state.timestamp/1_000_000/365, 1e-9)
                iv = BlackScholes.implied_volatility(mid, umid, self.params_opt[vp2]["strike"], tte)
                g  = BlackScholes.gamma(umid, self.params_opt[vp2]["strike"], tte, iv)
                va = BlackScholes.vanna(umid, self.params_opt[vp2]["strike"], tte, iv)
                w = self.params_opt[vp2]["vol_window"]
                for hist_key, val in [("past_iv",iv),("past_gamma",g),("past_vanna",va)]:
                    h = opt[vp2][hist_key]; h.append(val)
                    if len(h)>w: opt[vp2][hist_key] = h[-w:]
                ivh, gh, vah = opt[vp2]["past_iv"], opt[vp2]["past_gamma"], opt[vp2]["past_vanna"]
                if len(ivh)>=w:
                    miv, siv = np.mean(ivh), np.std(ivh)
                    mg, mav = np.mean(gh), np.mean(np.abs(vah))
                    fair = BlackScholes.black_scholes_call(umid, self.params_opt[vp2]["strike"], tte, miv)
                    logger.print(f"[{vp2}] Mid={mid:.2f}, Fair={fair:.2f}, IV={iv:.4f}")
                    pb = mid < fair
                    pa = iv > (miv + IV_STD_DEV_MULTIPLIER*max(siv, MIN_IV_STD_DEV_THRESHOLD))
                    gok = g > GAMMA_MEAN_FACTOR*mg; vok=abs(va)>VANNA_MEAN_FACTOR*mav
                    sig = 0
                    if pb and gok and vok: sig = -1
                    elif pa and mid>fair and gok and vok: sig = 1
                    elif (pb or pa):
                        rr=[]
                        if not gok: rr.append("Gamma low")
                        if not vok: rr.append("Vanna low")
                        logger.print(f"[{vp2}] Suppressed: {','.join(rr)}")
                    if sig:
                        ords=[]; pos = state.position.get(vp2,0); lim=self.LIMIT_OPT[vp2]
                        if sig==-1:
                            need = lim - pos
                            price = ba
                            avail = sum(abs(v) for p,v in od_v.sell_orders.items() if p<=price)
                            q = min(need, avail)
                            if q>0:
                                ords.append(Order(vp2, price, q))
                                logger.print(f"BUY {vp2}: {q}@{price}")
                        else:
                            need = abs(-lim - pos)
                            price = bb
                            avail = sum(abs(v) for p,v in od_v.buy_orders.items() if p>=price)
                            ...
                            if q>0:
                                ords.append(Order(vp2, price, -q))
                                logger.print(f"SELL {vp2}: {q}@{price}")
                        if ords: combined[vp2] = ords
                else:
                    logger.print(f"[{vp2}] Warming up ({len(ivh)}/{w})")
            else:
                logger.print(f"[{vp2}] Skipped: missing orders")

        full["options"] = opt

        # --- PAIR STRATEGY: SQUID_INK/KELP ---
        spot = full["spot"]
        ...
        if pair and Product.SQUID_INK in pair:
            combined[Product.SQUID_INK] = pair[Product.SQUID_INK]
        full["spot"] = spot

        # emit
        out_td = jsonpickle.encode(full)
        ...
        if Product.VOLCANIC_ROCK in state.order_depths:
            ...
            rock_orders = self.mean_reversion_orders(
                Product.VOLCANIC_ROCK, rock_order_depth, rock_position, full["spot"]
            )
            if rock_orders:
        ...
        basket_prods = [Product.PICNIC_BASKET1, Product.PICNIC_BASKET2, Product.DJEMBES]
        if all(p in state.order_depths for p in basket_prods):
        ...
        cj_prods = [Product.CROISSANTS, Product.JAMS]
        if all(p in state.order_depths for p in cj_prods):
            croissant_mid = self.get_swmid(od[Product.CROISSANTS])
            jam_mid = self.get_swmid(od[Product.JAMS])
            ...
            # Exit condition: If z-score falls below the exit threshold, we exit the trade
                ...
                orders2[Product.CROISSANTS].append(Order(Product.CROISSANTS, int(croissant_mid), 1 * qty)) #2
                orders2[Product.JAMS].append(Order(Product.JAMS, int(jam_mid), -1 * qty)) #SHOULD WE INCREQSE QUANTITY ?
            ...
            for product, ords in combined.items():
                if ords: 
                    result.setdefault(product, []).extend(ords)
        else:
            pass
        traderData = jsonpickle.encode(full)
