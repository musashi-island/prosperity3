...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        pos = state.position
        required_basket = [Product.PICNIC_BASKET1, Product.PICNIC_BASKET2, Product.DJEMBES]
        if all(p in od for p in required_basket):
            ...
            if None not in [b1_mid, b2_mid, djembe_mid]:
                left = 2 * b1_mid - 3 * b2_mid
                right = 2 * djembe_mid
                spread = left - right
                ...
                qty = 1
                if z > self.z_entry:
                    result.setdefault(Product.PICNIC_BASKET2, []).append(Order(Product.PICNIC_BASKET2, int(b2_mid), 3 * qty))
                    result.setdefault(Product.PICNIC_BASKET1, []).append(Order(Product.PICNIC_BASKET1, int(b1_mid), -2 * qty))
                    result.setdefault(Product.DJEMBES, []).append(Order(Product.DJEMBES, int(djembe_mid), 2 * qty))
                elif z < -self.z_entry:
                    result.setdefault(Product.PICNIC_BASKET2, []).append(Order(Product.PICNIC_BASKET2, int(b2_mid), -3 * qty))
                    result.setdefault(Product.PICNIC_BASKET1, []).append(Order(Product.PICNIC_BASKET1, int(b1_mid), 2 * qty))
                    result.setdefault(Product.DJEMBES, []).append(Order(Product.DJEMBES, int(djembe_mid), -2 * qty))
        """
        # --- CROISSANT & JAM Spread Strategy ---
        if Product.CROISSANTS in od and Product.JAMS in od:
            croissant_mid = self.get_swmid(od[Product.CROISSANTS])
            jam_mid = self.get_swmid(od[Product.JAMS])
            if None not in [croissant_mid, jam_mid]:
                ...
                qty = 1
                if z > self.z_entry2:
                    result.setdefault(Product.CROISSANTS, []).append(Order(Product.CROISSANTS, int(croissant_mid), -qty))
                    result.setdefault(Product.JAMS, []).append(Order(Product.JAMS, int(jam_mid), qty))
